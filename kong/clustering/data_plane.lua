  ...
  local config_thread = ngx.thread.spawn(function()
    while not exiting() and not config_exit do
      ...
      if ok then
        ...
        if data then
          local finish = kong.profiling.start("decompress payload of size ", #data, " bytes")
          local msg = assert(inflate_gzip(data))
          finish()

          yield()

          finish = kong.profiling.start("json decode payload of size ", #msg, " bytes")
          ...
          if msg.type == "reconfigure" then
            finish = kong.profiling.start("reconfigure frame received")
            ...
            if config_table then
              local done = kong.profiling.start("update config")
