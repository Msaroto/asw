  ...
  describe("compile_kong_gui_include_conf()", function ()
    describe("admin_gui_path", function ()
      it("set admin_gui_path to /", function ()
        ...
        assert.matches("sub_filter '/__admin_gui_base__/' '/';", kong_gui_include_conf)
      end)
      it("set admin_gui_path to /canopy", function ()
        local conf = assert(conf_loader(nil, {
          admin_gui_path = "/canopy",
        }))
        local kong_gui_include_conf = prefix_handler.compile_kong_gui_include_conf(conf)
        assert.matches("location%s+=%s+/canopy/kconfig%.js", kong_gui_include_conf)                 -- location = /canopy/kconfig.js
        assert.matches("location%s+~%*%s+%^/canopy%(%?<path>/%.%*%)%?%$", kong_gui_include_conf)    -- location ~* ^/canopy(?<path>/.**)?$
        assert.matches("sub_filter%s+'/__admin_gui_base__/'%s+'/canopy/';", kong_gui_include_conf)  -- sub_filter '/__admin_gui_base__/' '/canopy/';
      end)
    end)
  end)
