...
describe("Configuration loader", function()
  it("loads the defaults", function()
    ...
    assert.same({"127.0.0.1:8002", "127.0.0.1:8445 ssl"}, conf.admin_gui_listen)
  ...
  it("extracts flags, ports and listen ips from proxy_listen/admin_listen/admin_gui_listen", function()
    ...
    assert.equal("127.0.0.1", conf.admin_gui_listeners[1].ip)
    ...
    assert.equal("127.0.0.1:8002", conf.admin_gui_listeners[1].listener)

    assert.equal("127.0.0.1", conf.admin_gui_listeners[2].ip)
    ...
    assert.equal("127.0.0.1:8445 ssl", conf.admin_gui_listeners[2].listener)
  ...
  it("should populate correct admin_gui_origin", function()
    ...
    conf, _, errors = conf_loader(nil, {
      admin_gui_url = "http://localhost:8002/canopy",
    })
    assert.is_nil(errors)
    assert.is_not_nil(conf)
    assert.is_not_nil(conf.admin_gui_origin)
