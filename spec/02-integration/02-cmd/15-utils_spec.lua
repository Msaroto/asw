...
describe("kong.cmd.utils.process", function()
  ...
  describe("pid()", function()
    ...
    it("prefers a pid file to a numeric string pid", function()
      ...
      local got, err = process.pid(fname)
      assert.is_nil(err)
      assert.equals(123, got)
    end)

    it("returns nil+error for non-number/non-string inputs", function()
      local pid, err
      pid, err = process.pid(nil)
      assert.is_nil(pid)
      assert.same(process.E_INVALID_PID_TYPE, err)

      pid, err = process.pid({})
      assert.is_nil(pid)
      assert.same(process.E_INVALID_PID_TYPE, err)

      pid, err = process.pid(ngx.null)
      assert.is_nil(pid)
      assert.same(process.E_INVALID_PID_TYPE, err)

      pid, err = process.pid(false)
      assert.is_nil(pid)
      assert.same(process.E_INVALID_PID_TYPE, err)
    end)

    it("returns nil+error for values less than 1", function()
      local pid, err

      pid, err = process.pid(-1)
      assert.is_nil(pid)
      assert.same(process.E_PID_OUT_OF_RANGE, err)

      pid, err = process.pid(0)
      assert.is_nil(pid)
      assert.same(process.E_PID_OUT_OF_RANGE, err)

      assert.truthy(pl_file.write(pid_file, "-1"))
      pid, err = process.pid(pid_file)
      assert.is_nil(pid)
      assert.same(process.E_PID_OUT_OF_RANGE, err)
  ...
  describe("pid_from_file()", function()
    ...
    it("returns nil+error on filesystem errors", function()
      ...
      local pid, err = process.pid_from_file(pid_file)
      assert.is_nil(pid)
      assert.same(process.E_NO_SUCH_PID_FILE, err)
    end)
    ...
    it("returns nil+error if the pid file is empty", function()
      local pid, err = process.pid_from_file(pid_file)
      assert.is_nil(pid)
      assert.same(process.E_EMPTY_PID_FILE, err)
    ...
    it("returns nil+error if the pid file contents are invalid", function()
      ...
      local pid, err = process.pid_from_file(pid_file)
      assert.is_nil(pid)
      assert.same(process.E_INVALID_PID_FILE, err)
    end)
  end)
