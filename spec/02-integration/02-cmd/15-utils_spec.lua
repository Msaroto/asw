...
describe("kong.cmd.utils.kill", function()
  local pid_file
  ...
  describe("guess_pid()", function()
    it("accepts a number", function()
      local pid, err = kill.guess_pid(123)
      assert.is_nil(err)
    ...
    it("accepts a pid filename", function()
      ...
      local pid, err = kill.guess_pid(pid_file)
      assert.is_nil(err)
    ...
    it("prefers a pid file to a numeric string pid", function()
      ...
      local got, err = kill.guess_pid(fname)
      assert.is_nil(err)
    ...
    it("throws an error for non-number/non-string inputs", function()
      ...
      assert.has_errors(function()
        kill.guess_pid(ngx.null)
      end)

      assert.has_errors(function()
        kill.guess_pid(false)
      end)
    end)
  ...
  describe("pid_from_file()", function()
    it("reads pid from a file", function()
      assert.truthy(pl_file.write(pid_file, "1234"))
      local pid, err = kill.pid_from_file(pid_file)
      assert.is_nil(err)
      assert.equals(1234, pid)
    end)
    ...
    it("returns nil+error if the pid file is empty", function()
      ...
      pid, err = kill.pid_from_file(pid_file)
      assert.is_nil(pid)
      assert.is_string(err)
      assert.matches("empty", err)
    end)
  ...
  describe("exists()", function()
    it("returns true for a pid of a running process", function()
      local exists, err = kill.exists(ngx.worker.pid())
      assert.is_nil(err)
    ...
    it("returns true for a pid file of a running process", function()
      ...
      local exists, err = kill.exists(pid_file)
      assert.is_nil(err)
      assert.is_true(exists)
    end)
    ...
    it("returns false for the pid file of a non-existent process", function()
      ...
      for _ = 1, 1000 do
        ...
        exists, err = kill.exists(pid_file)
        if exists == false then
          break
        end
      end
    ...
    it("returns nil+error when a pid file does not exist", function()
      ...
      local pid, err = kill.exists(pid_file)
      assert.is_nil(pid)
      assert.is_string(err)
      assert.matches("No such file", err)
    end)
  ...
  describe("signal()", function()
    ...
    local function spawn()
      ...
      assert.is_true(kill.exists(pid), "failed to spawn process")
    ...
    it("sends a signal to a running process, using a pid", function()
      ...
      local ok, err = kill.signal(pid, 15)
      assert.is_nil(err)
      assert.truthy(ok)
      ...
      assert.is_false(kill.exists(pid))
    end)
    ...
    it("sends a signal to a running process, using a pid file", function()
      ...
      local ok, err = kill.signal(pid_file, 15)
      assert.is_nil(err)
      assert.truthy(ok)
      ...
      assert.is_false(kill.exists(pid_file))
    end)
    ...
    it("accepts a signal name in place of signum", function()
      ...
      local ok, err = kill.signal(pid, "INT")
      assert.is_nil(err)
      assert.truthy(ok)
